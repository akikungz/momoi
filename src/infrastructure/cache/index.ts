...
export class CacheModule {
	...
	public async createCacheKey(
		...
		try {
			...
			this.markUnavailable(
				"Failed to write to Redis cache. Continuing without cache.",
				error,
			);
		}
	}
	...
	public async getCacheValue(key: string): Promise<string | null> {
		...
		try {
			...
			this.markUnavailable(
				"Failed to read from Redis cache. Continuing without cache.",
				error,
			);
			return null;
		}
	...
	public async deleteCacheKey(key: string) {
		...
		try {
			...
			this.markUnavailable(
				"Failed to delete Redis cache key. Continuing without cache.",
				error,
			);
		}
	}
	...
	public async deleteCacheByPattern(pattern: string) {
		...
		try {
			...
			stream.on("data", (keys: string[]) => {
				if (keys.length) {
					...
					pipeline.exec().catch((error) => {
						this.markUnavailable(
							"Failed to delete Redis cache keys by pattern. Continuing without cache.",
							error,
						);
					});
				}
			...
			stream.on("error", (error) => {
				this.markUnavailable(
					"Failed to scan Redis cache keys by pattern. Continuing without cache.",
					error,
				);
			});
		} catch (error) {
			this.markUnavailable(
				"Failed to invalidate Redis cache by pattern. Continuing without cache.",
				error,
			);
		}
	}
	...
	public async flushAll() {
		...
		try {
			...
			this.markUnavailable(
				"Failed to flush Redis cache. Continuing without cache.",
				error,
			);
		}
	}
