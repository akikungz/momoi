...
app.listen(env.PORT, ({ port, hostname }) => {
	...
	console.log(
		`Server is running at http://${hostname || "localhost"}:${port}/api`,
	);
});
...
async function gracefulShutdown(reason: string, exitCode: number = 0) {
	...
	const forceExitTimeout = setTimeout(() => {
		emitLog(SeverityNumber.ERROR, "ERROR", "Graceful shutdown timed out", {
			reason,
		});
		process.exit(1);
	}, 15_000);
	...
	emitLog(SeverityNumber.INFO, "INFO", "Graceful shutdown completed", {
		reason,
	});
