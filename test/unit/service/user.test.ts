...
describe("UserService", () => {
	...
	describe("addSSHKey", () => {
		...
		it("should throw error on unexpected error", async () => {
			...
			mockPrisma.platformSSHKey.create.mockRejectedValueOnce(
	...
	describe("removeSSHKey", () => {
		...
		it("should throw error when SSH key not found (P2025)", async () => {
			...
			try {
				...
				expect((err as Error).message).toBe(
					"One or more SSH keys not found for the user.",
				);
			}
		});
		...
		it("should throw error on database error", async () => {
			...
			const error = new PrismaClientKnownRequestError(
				"Database error",
				{ code: "P2003", clientVersion: "0.0.1" }, // Use P2003 (foreign key constraint) which triggers default database error
			);
			mockPrisma.platformSSHKey.deleteMany.mockRejectedValueOnce(error);
		...
		it("should throw error on unexpected error", async () => {
			...
			mockPrisma.platformSSHKey.deleteMany.mockRejectedValueOnce(
				new Error("Unexpected error"),
			);
			...
			try {
				...
				expect((err as Error).message).toBe(
					"An unexpected error occurred while removing the SSH keys.",
				);
			}
		});
	});
