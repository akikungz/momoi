...
import { authHandler, authMacro, authOpenAPI } from "./infrastructure/auth";
import { CacheModule } from "./infrastructure/cache";
import { prisma } from "./infrastructure/database";
import { env } from "./infrastructure/env";
import { QueueModule } from "./infrastructure/queue";
import { createObjectStorageProvider } from "./infrastructure/storage";
import {
	...
	httpMetrics,
	SeverityNumber,
} from "./infrastructure/telemetry/runtime";
...
export const api = new Elysia({
		...
		openapi({
			documentation: {
				...
				tags: [
					...
					{
						name: "Mailing List",
						description: "Endpoints for managing mailing lists",
					},
					{
						name: "Requests",
						description: "Instance request management endpoints",
					},
					{
						name: "Extended Requests",
						description: "Extended instance request management endpoints",
					},
					{
						name: "Reverse Proxy",
						description: "Endpoints for managing instance reverse proxies",
					},
					{
						name: "Audit Logs",
						description: "Endpoints for viewing audit history",
					},
					{
						name: "Storage",
						description: "File and folder management endpoints",
					},
					{ name: "File Versions", description: "File versioning endpoints" },
					{
						name: "File Permissions",
						description: "File sharing and permission endpoints",
					},
					{
						name: "Autocomplete",
	...
	.onAfterResponse(({ request, path, set }) => {
		...
		const requestStartedAt =
			telemetryState?.requestStartedAt ?? performance.now();
