...
describe("Academic Route", () => {
	...
	it("should return cached instructors list", async () => {
		...
		const response = await client.academic.instructors.get({
			query: { page: 1, pageSize: 10 },
		});
	...
	it("should add mailing entry", async () => {
		...
		const addRes = await client.academic["mailing-list"].post({
			email: "new@example.com",
		});
	...
	it("should remove mailing entry", async () => {
		...
		const delRes = await client.academic["mailing-list"]({
			mailingId: 9,
		}).delete();
	...
	it("should edit instructor", async () => {
		...
		const editRes = await client.academic
			.instructors({ instructorId: 4 })
			.patch({ role: "ADMIN", courseIds: [course.id] });
	...
	it("should edit course instructors", async () => {
		...
		const instRes = await client.academic
			.courses({ courseId: 1 })
			.instructors.patch({ instructorIds: [1] });
	...
	it("should edit course semesters", async () => {
		...
		const semRes = await client.academic
			.courses({ courseId: 1 })
			.semesters.patch({ semesterIds: semesters.map((s) => s.id) });
	...
	it("should edit semester courses", async () => {
		...
		const response = await client.academic
			.semesters({ semesterId: 1 })
			.courses.patch({
	...
	it("should list semesters", async () => {
		...
		const listRes = await client.academic.semesters.get({
			query: { page: 1, pageSize: 10 },
		});
	...
	it("should get semester by id", async () => {
		...
		const getRes = await client.academic
			.semesters({ semesterId: sem.id })
			.get();
		expect(getRes.status).toBe(200);
	...
	it("should get current semester as instructor", async () => {
		const client = treaty(
			academicRoute(mockPrisma, mockCache as any, mockInstructorAuth),
		);
	...
	it("should add semester", async () => {
		...
		const addRes = await client.academic.semesters.post({
			name: sem.name,
	...
	it("should forbid student access", async () => {
		const client = treaty(
			academicRoute(mockPrisma, mockCache as any, mockStudentAuth),
		);
	...
	it("should return 401 for unauthorized user", async () => {
		const client = treaty(
			academicRoute(mockPrisma, mockCache as any, mockOtherAuth),
		);
