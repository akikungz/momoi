...
describe("StorageService", () => {
  ...
  beforeEach(() => {
    ...
    objectStorage = {
      kind: "s3",
      enabled: true,
      createObjectKey: mock((ownerId: number, filename?: string) => `user/${ownerId}/${filename ?? "file.bin"}`),
    ...
    storageService = new StorageService(mockPrisma, mockCache as any, objectStorage);
  });
  ...
  it("creates presigned upload url for new file", async () => {
    ...
    expect(objectStorage.createUploadUrl).toHaveBeenCalledWith("user/1/notes.pdf", "application/pdf");
  });
  ...
  it("rejects upload url creation when provider is disabled", async () => {
    ...
    await expect(storageService.createUploadUrl(instructorUser, {
      filename: "notes.pdf",
      contentType: "application/pdf",
  ...
  it("creates version upload url only for file owner", async () => {
    ...
    const result = await storageService.createVersionUploadUrl(instructorUser, "file-1", {
  ...
  it("creates latest version download url from provider", async () => {
    ...
    const result = await storageService.createDownloadUrl(instructorUser, "file-2");

    expect(result.objectKey).toBe("objects/file-2-v3");
    expect(result.downloadUrl).toContain("https://storage.example/download/");
    expect(objectStorage.createDownloadUrl).toHaveBeenCalledWith("objects/file-2-v3");
  ...
  it("returns cached latest version download url when available and not expired", async () => {
    ...
    mockCache.getCacheValue.mockResolvedValueOnce(JSON.stringify({
      url: "https://storage.example/download/cached-url",
      expiresAt: cachedExpiresAt.toISOString(),
    }));

    const result = await storageService.createDownloadUrl(instructorUser, "file-2-cache");

    expect(result.objectKey).toBe("objects/file-2-cache-v1");
    expect(result.downloadUrl).toBe("https://storage.example/download/cached-url");
  ...
  it("caches version download url with ttl based on expiresAt", async () => {
    ...
    await storageService.createVersionDownloadUrl(instructorUser, "file-5-ttl", 11);
  ...
  it("falls back to storage path when provider is disabled for download", async () => {
    ...
    const result = await storageService.createDownloadUrl(instructorUser, "file-3");
  ...
  it("blocks download url when user has no access", async () => {
    ...
    await expect(storageService.createDownloadUrl(instructorUser, "file-4"))
  ...
  it("creates version download url for permitted user", async () => {
    ...
    const result = await storageService.createVersionDownloadUrl(instructorUser, "file-5", 10);

    expect(result.objectKey).toBe("objects/file-5-v10");
    expect(objectStorage.createDownloadUrl).toHaveBeenCalledWith("objects/file-5-v10");
  });
  ...
  it("deletes version and object key when s3 provider is enabled", async () => {
    ...
    const result = await storageService.deleteVersion(instructorUser, "file-6", 11);

    expect(result.success).toBe(true);
    expect(mockPrisma.platformFileVersion.delete).toHaveBeenCalledWith({ where: { id: 11 } });
    expect(objectStorage.deleteObject).toHaveBeenCalledWith("objects/file-6-v11");
  });

  it("throws error when file not found", async () => {
    mockPrisma.platformFile.findUnique.mockResolvedValueOnce(null);

    await expect(storageService.createDownloadUrl(instructorUser, "nonexistent"))
  ...
  it("throws error when version not found", async () => {
    ...
    await expect(storageService.createVersionDownloadUrl(instructorUser, "file-7", 99))
  ...
  it("forbids access to trashed files", async () => {
    ...
    await expect(storageService.createDownloadUrl(instructorUser, "file-8"))
      .rejects.toThrow("File or latest version not found.");
  });
  ...
  it("forbids access to deleted files", async () => {
    ...
    await expect(storageService.createDownloadUrl(instructorUser, "file-9"))
      .rejects.toThrow("File or latest version not found.");
  });
  ...
  it("allows PUBLIC file download without permission", async () => {
    ...
    const result = await storageService.createDownloadUrl(instructorUser, "file-10");
  ...
  it("returns empty results for files search with no matches", async () => {
    ...
    const result = await storageService.searchFiles(instructorUser, { query: "nonexistent", page: 1, pageSize: 10 });
  ...
  it("gets file detail with metadata", async () => {
    ...
    const result = await storageService.getFileDetail(instructorUser, "file-11");
  ...
  it("forbids non-owner from deleting file", async () => {
    ...
    await expect(storageService.deleteFile(instructorUser, "file-14"))
  ...
  it("creates permission for file owner", async () => {
    ...
    const result = await storageService.createPermission(instructorUser, "file-15", {
      email: "viewer@example.com",
      permission: "VIEWER",
    });
  ...
  it("updates permission as file owner", async () => {
    ...
    const result = await storageService.updatePermission(instructorUser, "file-16", 1, {
      permission: "EDITOR",
    });
  ...
  it("blocks students from uploading files", async () => {
    ...
    await expect(storageService.createUploadUrl(studentUser, {
