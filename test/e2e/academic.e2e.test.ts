...
import { createMockCourse, createMockSemester } from "@test/mocks";
		...
		describe("DELETE /api/academic/mailing-list/:mailingId", () => {
			describe("As Admin", () => {
				it("should remove instructor from mailing list", async () => {
					...
					const response = await client.api.academic["mailing-list"]({
						mailingId: 1,
					}).delete();
		...
		describe("GET /api/academic/instructors/:instructorId", () => {
			describe("As Admin", () => {
				it("should return instructor details with courses", async () => {
					...
					const response = await client.api.academic
						.instructors({ instructorId: 1 })
						.get();
		...
		describe("GET /api/academic/semesters/current", () => {
			describe("As Admin", () => {
				...
				it("should auto-detect current semester by date range", async () => {
					...
					const now = new Date();
					const autoDetectedSemester = createMockSemester({
						id: 2,
						name: "Spring 2026",
						isCurrent: false,
					...
					expect(response.status).toBe(200);
					expect(response.data).toBeDefined();
					expect(response.data?.name).toBe("Spring 2026");
					expect(response.data?.isCurrent).toBe(true);
				});
