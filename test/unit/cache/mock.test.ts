...
describe("MockCache", () => {
  ...
  describe("createCacheKey", () => {
    ...
    it("should accept key and value parameters", async () => {
      ...
      expect(mockCache.createCacheKey).toHaveBeenCalledWith("test-key", "test-value");
    });
    ...
    it("should be mockable to throw errors", async () => {
      mockCache.createCacheKey.mockRejectedValueOnce(new Error("Cache write failed"));

      await expect(mockCache.createCacheKey("key", "value")).rejects.toThrow("Cache write failed");
    });
  });
  ...
  describe("deleteCacheByPattern", () => {
    ...
    it("should be mockable to throw errors", async () => {
      mockCache.deleteCacheByPattern.mockRejectedValueOnce(new Error("Delete failed"));

      await expect(mockCache.deleteCacheByPattern("pattern")).rejects.toThrow("Delete failed");
    });
  });
  ...
  describe("deleteCacheKey", () => {
    ...
    it("should be mockable to throw errors", async () => {
      mockCache.deleteCacheKey.mockRejectedValueOnce(new Error("Delete key failed"));

      await expect(mockCache.deleteCacheKey("key")).rejects.toThrow("Delete key failed");
    });
  });
  ...
  describe("disconnect", () => {
    ...
    it("should be mockable to throw errors", async () => {
      mockCache.disconnect.mockRejectedValueOnce(new Error("Disconnect failed"));
