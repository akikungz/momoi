...
import type { MockAuth } from "./mock";
	...
	advanced: {
		...
		...(env.NODE_ENV === "production"
			...
			: {
					...
					useSecureCookies:
						env.BETTER_AUTH_URL?.startsWith("https://") || false,
				}),
	},
	logger: {
		disabled: false,
		...
		customSession(async ({ user, session }) => {
			...
			if (!platfromUser) {
				...
				if (isInstructorEmail(user.email)) {
					...
					if (!findMailListing) {
						throw new Error(
							"Unauthorized: Instructor email not found in listing",
						);
					}
			...
			getSchema().then(({ paths }) => {
				...
				for (const path of Object.keys(paths)) {
					...
					for (const method of Object.keys(paths[path])) {
						// biome-ignore lint/suspicious/noExplicitAny: OpenAPI schema is dynamic and we need to access the operation object to add tags
				...
				return reference;
				// biome-ignore lint/suspicious/noExplicitAny: OpenAPI schema is dynamic and we need to access the paths object
			}) as Promise<Record<string, any>>,
		components: getSchema().then(({ components }) => components) as Promise<
			Awaited<ReturnType<typeof _auth.api.generateOpenAPISchema>>["components"]
		>,
	} as const;
	...
	.macro({
		auth: {
			resolve: async ({ status, request: { headers }, cache, cookie }) => {
				...
				const cookie_token = env.BETTER_AUTH_URL?.startsWith("https://")
				...
				if (!session) {
					return status(401, {
						status: 401,
						message: "Unauthorized: No active session or invalid account",
					});
				}
...
export const authGuard = (macro: typeof authMacro | MockAuth = authMacro) =>
			...
			isInstructor: {
				resolve: async ({ user, status }) => {
					...
					if (user.role === "STUDENT") {
						return status(403, {
							status: 403,
							message: "Forbidden: Instructors and Admins only",
						});
					}
