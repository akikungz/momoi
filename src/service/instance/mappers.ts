...
function mapVmDetails(
	pveVM: {
		hostname: string;
		status: string;
		pveNetworkIP: { ipAddress: string } | null;
	} | null,
 ...
 */
export function mapInstanceToListItem(
	instance: InstanceListResult,
): Static<typeof GetInstancesResponse>["values"][number] {
	return {
		id: instance.id,
 ...
 */
export function mapInstanceToDetail(
	instance: InstanceDetailResult,
): Static<typeof GetInstanceResponse> {
	return {
		id: instance.id,
		owner: mapOwner(instance.platformUserId, instance.platformUser),
		courseOffering: mapCourseOffering(instance.courseOffering),
 ...
 */
export function mapInstanceCreateToResponse(
	instance: InstanceCreateResult,
): Static<typeof CreateInstanceResponse> {
	return {
		id: instance.id,
		courseOffering: mapCourseOffering(instance.courseOffering),
		status: instance.status,
