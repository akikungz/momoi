...
const ip_without_gateway = (cidr: string, gateway: string) => {
  ...
  const totalHosts = Math.pow(2, 32 - Number(prefixLength));
  ...
  for (let i = 0; i < totalHosts - 1; i++) {
    ...
    if (ipString !== gateway) {
      if (ipString.split(".")[3] !== "0" && ipString.split(".")[3] !== String(totalHosts - 1)) // Exclude network and broadcast addresses
...
const seed_pve = async () => {
  ...
  const pve_networks = await prisma.pVENetwork.createMany({
    data: [
      { name: "831", subnet: "10.20.31.0/24", gateway: "10.20.31.1", bridge: "vmbr1", vlanTag: 831 },
      { name: "832", subnet: "10.20.32.0/24", gateway: "10.20.32.1", bridge: "vmbr1", vlanTag: 832 },
      { name: "833", subnet: "10.20.33.0/24", gateway: "10.20.33.1", bridge: "vmbr1", vlanTag: 833 },
    ],
    skipDuplicates: true,
  });
