...
const sdk = new NodeSDK({
  ...
  spanProcessors: [
    new BatchSpanProcessor(createTraceExporter()),
  ],
  metricReaders: [
...
const observeMetricPoints = (
  result: { observe: (value: number, attributes?: TelemetryAttributes) => void; },
  state: MetricPointMap
) => {
...
const seedDefaultMetricPoints = () => {
  if (customMetricState.httpRequestCount.size === 0) {
    addMetricPoint(customMetricState.httpRequestCount, 0, defaultHttpCompletedAttributes);
  }

  if (customMetricState.httpRequestDuration.size === 0) {
    addMetricPoint(customMetricState.httpRequestDuration, 0, defaultHttpCompletedAttributes);
  }

  if (customMetricState.httpActiveRequests.size === 0) {
    addMetricPoint(customMetricState.httpActiveRequests, 0, defaultHttpActiveAttributes);
  }
...
export const httpMetrics = {
  ...
  recordResult(durationMs: number, statusCode: number, attributes: TelemetryAttributes = {}) {
    ...
    const aggregateAttributes = getHttpAggregateAttributes(attributes, statusCode);
    addMetricPoint(customMetricState.httpRequestByStatusClass, 1, aggregateAttributes);
    ...
    if (statusCode >= 400) {
      addMetricPoint(customMetricState.httpRequestErrors, 1, aggregateAttributes);
    }
  },
};
  ...
  get activeRequests() {
    return getOrCreateInstrument("app.runtime.node.active_requests", () =>
      getMeter().createObservableGauge("app.runtime.node.active_requests", {
        description: "Number of active internal requests held by the Node.js process",
      })
    );
  },
  get dependencyAvailability() {
    return getOrCreateInstrument("app.runtime.dependency.available", () =>
      getMeter().createObservableGauge("app.runtime.dependency.available", {
        description: "Availability of key app dependencies, reported as 1 for available and 0 for unavailable",
      })
    );
  },
  ...
  get heartbeatTime() {
    return getOrCreateInstrument("app.runtime.heartbeat_time", () =>
      getMeter().createObservableGauge("app.runtime.heartbeat_time", {
        description: "Current Unix timestamp emitted by the app telemetry heartbeat",
        unit: "s",
      })
    );
  },
  get configInfo() {
    return getOrCreateInstrument("app.config.info", () =>
      getMeter().createObservableGauge("app.config.info", {
        description: "Static app configuration info exported as a marker metric",
      })
    );
  },
  ...
  get dependencyConfigured() {
    return getOrCreateInstrument("app.dependency.configured", () =>
      getMeter().createObservableGauge("app.dependency.configured", {
        description: "Whether an app dependency is configured, reported as 1 for configured and 0 for not configured",
      })
    );
  },
};
...
const registerRuntimeMetrics = () => {
  ...
  applicationMetrics.dependencyAvailability.addCallback((result) => {
    for (const [dependency, available] of Object.entries(dependencyAvailability)) {
  ...
  getMeter()
    ...
    .addCallback((result) => observeMetricPoints(result, customMetricState.httpRequestCount));
  ...
  getMeter()
    ...
    .addCallback((result) => observeMetricPoints(result, customMetricState.httpRequestDuration));

  getMeter()
    .createObservableGauge("http.server.request.errors", {
      description: "Total number of HTTP requests that completed with an error status",
    })
    .addCallback((result) => observeMetricPoints(result, customMetricState.httpRequestErrors));

  getMeter()
    .createObservableGauge("http.server.request.by_status_class", {
      description: "Total number of HTTP requests by status class",
    })
    .addCallback((result) => observeMetricPoints(result, customMetricState.httpRequestByStatusClass));
  ...
  getMeter()
    ...
    .addCallback((result) =>
      observeDurationPercentiles(result, customMetricState.httpRequestDurationSamples, 0.5)
    );
  ...
  getMeter()
    ...
    .addCallback((result) =>
      observeDurationPercentiles(result, customMetricState.httpRequestDurationSamples, 0.95)
    );
  ...
  getMeter()
    ...
    .addCallback((result) =>
      observeDurationPercentiles(result, customMetricState.httpRequestDurationSamples, 0.99)
    );
  ...
  getMeter()
    ...
    .addCallback((result) => observeMetricPoints(result, customMetricState.storageBytes));
