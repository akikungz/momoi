...
describe("Autocomplete Route", () => {
	...
	describe("GET /autocomplete/courses", () => {
		...
		it("should filter courses by search term", async () => {
			...
			const response = await client.autocomplete.courses.get({
				query: { search: "CS" },
			});
	...
	describe("Authentication", () => {
		it("should return 401 for unauthenticated user", async () => {
			const client = treaty(
				autocompleteRoute(mockPrisma, mockCache, mockOtherAuth),
			);
		...
		it("should allow student access to autocomplete", async () => {
			const client = treaty(
				autocompleteRoute(mockPrisma, mockCache, mockStudentAuth),
			);
