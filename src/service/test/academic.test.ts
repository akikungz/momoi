...
import {
  createMockCourse, createMockPlatformUser, createMockPrisma, createMockSemester,
...
describe("AcademicService", () => {
  ...
  describe("Instructor mailing list", () => {
    it("should list mailing entries with pagination", async () => {
      ...
      mockPrisma.instructorSearch.findMany.mockResolvedValueOnce([
        { id: 1, email: "john@example.com", createdAt: new Date(), updatedAt: new Date() },
      ]);

      const result = await service.getInstructorMailingList({ page: 1, pageSize: 10 });
    ...
    it("should return cached mailing entries when present", async () => {
      const cached = { values: [], totalItems: 0, totalPages: 0, currentPage: 1, pageSize: 10 };
    ...
    it("should add mailing entry and clear cache", async () => {
      const created = { id: 1, email: "john@example.com", createdAt: new Date(), updatedAt: new Date() };
      mockPrisma.instructorSearch.create.mockResolvedValueOnce(created);

      const result = await service.addInstructorMailingList({ email: created.email });

      expect(result.email).toBe(created.email);
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:mailing:*');
    });

    it("should throw on duplicate mailing entry (P2002)", async () => {
      const error = new PrismaClientKnownRequestError("dup", { code: "P2002", clientVersion: "0.0.1" });
      mockPrisma.instructorSearch.create.mockRejectedValueOnce(error);

      await expect(service.addInstructorMailingList({ email: "john@example.com" })).rejects.toThrow("This email is already in the mailing list.");
    });
    ...
    it("should throw error when removing non-existent mailing entry", async () => {
      const error = new PrismaClientKnownRequestError("not found", { code: "P2025", clientVersion: "0.0.1" });
      mockPrisma.instructorSearch.delete.mockRejectedValueOnce(error);

      await expect(service.removeInstructorMailingList(999)).rejects.toThrow("Mailing list entry not found.");
    });
  });
  ...
  describe("Instructors", () => {
    ...
    it("should return cached instructor when present", async () => {
      const cached = { id: 5, name: "Jane", email: "jane@example.com", role: "INSTRUCTOR", courses: [], createdAt: new Date(), updatedAt: new Date() };
      mockCache.getCacheValue.mockResolvedValueOnce(JSON.stringify(cached));
    ...
    it("should throw error when instructor not found", async () => {
      ...
      await expect(service.getInstructorById(999)).rejects.toThrow("Instructor not found.");
    });

    it("should edit instructor and clear cache", async () => {
      const course = createMockCourse({ id: 5 });
      mockPrisma.platformUser.findUnique.mockResolvedValueOnce({ courses: [{ id: course.id }] });
      ...
      const result = await service.editInstructorById(1, { role: "ADMIN", courseIds: [5] });
    ...
    it("should throw error when editing non-existent instructor", async () => {
      ...
      await expect(service.editInstructorById(999, { role: "ADMIN" })).rejects.toThrow("Instructor not found.");
    });
  });
  ...
  describe("Courses", () => {
    ...
    it("should get course by id", async () => {
      ...
      mockPrisma.course.findUnique.mockResolvedValueOnce({
        ...course,
        instructors: [{ id: 7, role: "INSTRUCTOR", createdAt: new Date(), updatedAt: new Date(), userId: "u-1", user: { name: "Prof", email: "prof@example.com" } }],
        courseOfferings: [
          { semester },
        ],
      });
    ...
    it("should return cached course when present", async () => {
      const cached = { id: 2, code: "CS101", title: "Test", description: undefined, instructors: [], semesters: [], isActive: true, createdAt: new Date(), updatedAt: new Date() };
      mockCache.getCacheValue.mockResolvedValueOnce(JSON.stringify(cached));
    ...
    it("should throw error when course not found", async () => {
      ...
      await expect(service.getCourseById(999)).rejects.toThrow("Course not found.");
    });
    ...
    it("should add course and clear cache", async () => {
      ...
      const result = await service.addCourse({ code: course.code, title: course.title, description: course.description ?? undefined });

      expect(result.id).toBe(course.id);
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:courses:*');
    });

    it("should edit course instructors", async () => {
      mockPrisma.course.findUnique.mockResolvedValueOnce({ instructors: [{ id: 2 }] });
      ...
      const result = await service.editCourseInstructors(1, { instructorIds: [1] });

      expect(result.instructors[0].email).toBe("prof@example.com");
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:instructors:*');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:courses:*');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:course:1');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:instructor:1');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:instructor:2');
    });
    ...
    it("should edit course by id and clear cache", async () => {
      ...
      const result = await service.editCourseById(1, { title: "Updated Title" });
    ...
    it("should throw error when editing non-existent course", async () => {
      ...
      await expect(service.editCourseById(999, { title: "Updated" })).rejects.toThrow("Course not found.");
    });
    ...
    it("should edit course semesters", async () => {
      ...
      const result = await service.editCourseSemesters(1, { semesterIds: semesters.map(s => s.id) });

      expect(result.semesters).toHaveLength(2);
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:courses:*');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:semesters:*');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:course:1');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:semester:1');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:semester:2');
    });

    it("should throw error when semester not found in editCourseSemesters", async () => {
      mockPrisma.semester.findMany.mockResolvedValueOnce([createMockSemester({ id: 1 })]);

      await expect(service.editCourseSemesters(1, { semesterIds: [1, 2] })).rejects.toThrow("One or more semesters not found.");
    });
  });
  ...
  describe("Semesters", () => {
    ...
    it("should return cached semester when present", async () => {
      const cached = { id: 2, name: "Fall 2024", startDate: new Date(), endDate: new Date(), isCurrent: true, courses: [], createdAt: new Date(), updatedAt: new Date() };
      mockCache.getCacheValue.mockResolvedValueOnce(JSON.stringify(cached));
    ...
    it("should throw error when semester not found", async () => {
      ...
      await expect(service.getSemesterById(999)).rejects.toThrow("Semester not found.");
    });
    ...
    it("should add semester and clear cache", async () => {
      ...
      const result = await service.addSemester({ name: semester.name, startDate: semester.startDate, endDate: semester.endDate });
    ...
    it("should edit semester by id and clear cache", async () => {
      ...
      const result = await service.editSemesterById(1, { name: "Updated Semester" });
    ...
    it("should throw error when editing non-existent semester", async () => {
      ...
      await expect(service.editSemesterById(999, { name: "Updated" })).rejects.toThrow("Semester not found.");
    });
    ...
    it("should edit semester courses", async () => {
      ...
      const result = await service.editSemesterCourses(1, { courseIds: courses.map(c => c.id) });

      expect(result.courses).toHaveLength(2);
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:courses:*');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:semester:1');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:course:1');
      expect(mockCache.deleteCacheByPattern).toHaveBeenCalledWith('academic:course:2');
    });

    it("should throw error when course not found in editSemesterCourses", async () => {
      mockPrisma.course.findMany.mockResolvedValueOnce([createMockCourse({ id: 1 })]);

      await expect(service.editSemesterCourses(1, { courseIds: [1, 2] })).rejects.toThrow("One or more courses not found.");
    });

    it("should delete semester and clear cache", async () => {
      mockPrisma.courseOffering.findMany.mockResolvedValueOnce([{ courseId: 1 }, { courseId: 2 }]);
    ...
    it("should throw error when deleting non-existent semester", async () => {
      ...
      await expect(service.deleteSemesterById(999)).rejects.toThrow("Semester not found.");
    });
  });
});
