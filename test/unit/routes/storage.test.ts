...
describe("Storage Route", () => {
  ...
  it("lists owned files", async () => {
    const client = treaty(storageRoute(mockPrisma, mockCache as any, mockAdminAuth, objectStorage));
    ...
    const response = await client.storage.files.get({ query: { page: 1, pageSize: 10 } });
  ...
  it("lists shared-with-me files when sharedWithMe query is true", async () => {
    ...
    const response = await client.storage.files.get({ query: { page: 1, pageSize: 10, sharedWithMe: true } });
  ...
  it("creates upload url via object storage provider", async () => {
    ...
    expect(response.data?.uploadUrl).toContain("https://storage.example/upload/");
  });
  ...
  it("creates latest version download url", async () => {
    ...
    const response = await client.storage.files({ fileId: "file-2" })["download-url"].get();

    expect(response.status).toBe(200);
    expect(response.data?.objectKey).toBe("objects/file-2-v1");
    expect(response.data?.downloadUrl).toContain("https://storage.example/download/");
  });
  ...
  it("returns forbidden when download access is denied", async () => {
    ...
    const response = await client.storage.files({ fileId: "file-3" })["download-url"].get();
  ...
  it("creates specific version download url when shared permission exists", async () => {
    ...
    const response = await client.storage.files({ fileId: "file-5" }).versions({ versionId: 7 })["download-url"].get();
  ...
  it("blocks student from creating files", async () => {
    const client = treaty(storageRoute(mockPrisma, mockCache as any, mockStudentAuth, objectStorage));
  ...
  it("returns 404 when file not found", async () => {
    ...
    const response = await client.storage.files({ fileId: "nonexistent" }).get();
  ...
  it("searches files with query", async () => {
    ...
    const response = await client.storage.files.search.get({ query: { query: "result", page: 1, pageSize: 10 } });
  ...
  it("deletes file", async () => {
    ...
    const response = await client.storage.files({ fileId: "delete-1" }).delete();
