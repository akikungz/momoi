...
describe("Instance Route - Admin", () => {
	...
	beforeEach(() => {
		...
		if (mockQueue.provisionInstanceQueue.add.mockReset)
			mockQueue.provisionInstanceQueue.add.mockReset();
		if (mockQueue.deprovisionInstanceQueue.add.mockReset)
			mockQueue.deprovisionInstanceQueue.add.mockReset();
	});

	it("should create a new instance as instructor", async () => {
		mockQueue.provisionInstanceQueue.add.mockResolvedValueOnce({
			id: "mock-job",
			name: "provision",
			data: {},
		});
		mockQueue.deprovisionInstanceQueue.add.mockResolvedValueOnce({
			id: "mock-job",
			name: "deprovision",
			data: {},
		});
		...
		const _mockCourseOffering = createMockCourseOffering({
			id: 1,
			courseId: 1,
			semesterId: 1,
		});
	...
	it("should get instances for user", async () => {
		...
		expect(response.data?.values[0]).toHaveProperty(
			"owner.email",
			"student@example.com",
		);
	...
	it("should handle cache for get instances", async () => {
		...
		const _cacheKey = `user:1:instances:page:1:size:10:courseId:all:semesterId:all`;
...
describe("Instance Route - Student", () => {
	...
	it("should get extended requests for own instance", async () => {
		...
		mockPrisma.extendedRequest.findMany.mockResolvedValueOnce(
			mockExtendedRequests,
		);

		const response = await client
			.instances({ instanceId: 50 })
