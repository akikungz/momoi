...
describe("RequestService", () => {
	...
	it("rejects cancel by non-requester", async () => {
		...
		await expect(
			requestService.updateRequestStatus(instructorUser, requestRecord.id, {
				status: "CANCELLED",
				reason: "nope",
			} as any),
	...
	it("updateRequestStatus returns 404 when request not found", async () => {
		...
		await expect(
			requestService.updateRequestStatus({ id: 1, role: "ADMIN" }, 12345, {
				status: "APPROVED",
				reason: "ok",
			} as any),
	...
	it("creates extended request when student owns instance", async () => {
		...
		expect(mockPrisma.extendedRequest.create).toHaveBeenCalledWith(
	...
	it("throws when no upcoming semester exists", async () => {
		...
		await expect(
			requestService.createExtendedRequest(ownerId, {
				title: "Extend",
				description: "more time",
				targetInstanceId: instance.id,
			} as any),
		).rejects.toThrow("No upcoming semester found for this extended request.");
	});
	...
	it("rejects extended request when not owner", async () => {
		...
		await expect(
			requestService.createExtendedRequest(5, {
				title: "Extend",
				description: "more time",
				targetInstanceId: 22,
			} as any),
		).rejects.toThrow("Instance not found or not owned by the user.");
	});
	...
	it("instructor can approve extended request for their course", async () => {
		...
		const result = await requestService.updateExtendedRequestStatus(
			{ id: 2, role: "INSTRUCTOR" },
			extendedRequest.id,
			{
				status: "APPROVED",
	...
	it("updateExtendedRequestStatus returns 404 when extended request not found", async () => {
		...
		await expect(
			requestService.updateExtendedRequestStatus(
				{ id: 1, role: "ADMIN" },
				99999,
				{
					status: "APPROVED",
		...
		).rejects.toThrow("Extended request not found.");
	});
	...
	it("updateExtendedRequestStatus returns 400 when extended request already processed", async () => {
		...
		).rejects.toThrow("Extended request has already been processed.");
	});
});
