...
describe("E2E: Storage Routes", () => {
	...
	describe("POST /api/storage/files/upload-url", () => {
		it("creates presigned upload url for admin", async () => {
			...
			expect(response.data?.uploadUrl).toContain(
				"https://e2e.storage.local/upload/",
			);
		});
	...
	describe("GET /api/storage/files/:fileId/download-url", () => {
		it("creates latest-version download url for owner", async () => {
			...
			expect(response.data?.downloadUrl).toContain(
				"https://e2e.storage.local/download/",
			);
		});
	...
	describe("File CRUD Operations", () => {
		it("updates file metadata", async () => {
			...
			const updateResponse = await client.api.storage
				.files({ fileId: "lifecycle-1" })
				.patch({
					name: "Updated File",
				});
		...
		it("deletes a file", async () => {
			...
			const deleteResponse = await client.api.storage
				.files({ fileId: "lifecycle-2" })
				.delete();
